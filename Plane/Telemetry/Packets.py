...
class PacketProtocol:
    def __init__(self, packetType=PacketType.DEFAULT, decoded=''):
        ...
        if isinstance(self.decoded, str):
            self.decoded = self.decoded.encode()
    ...
    def decode(self, packetized):
        headerSize = struct.calcsize(self.header_format)
        footerSize = struct.calcsize("!H")

        self.crc16 = packetized[len(packetized) - footerSize - 1:len(packetized) - 1]
        newCRC16 = self.calculateCRC16(packetized[:len(packetized) - footerSize - 1])

        if self.crc16 == newCRC16:
            header = packetized[:headerSize]
            ...
            self.decoded = packetized[headerSize:headerSize + self.header['decodedLen']].decode()
            return True
